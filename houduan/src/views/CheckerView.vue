<template>
  ...
  <div style="margin:10px">
    <el-button @click="add">新增审核员</el-button>
    <el-button>导入</el-button>
    <el-button>导出</el-button>

  </div>
  <div style="margin:10px">
    <el-input v-model="searchName" placeholder="查询审核员姓名" style="width: 20% "/>
    <el-input v-model="searchNum" placeholder="查询审核员工号" style="width: 20%;margin: 10px"/>
  ...
  methods: {
    ...
    save(){
      if(this.user.id){//update
        request.put("/updateChecker",this.user).then(res=>{
          console.log(res);
          if(res.code=='0'){
            this.$message({
              type:"success",
        ...
        request.post("/addChecker",this.user).then(res=>{
